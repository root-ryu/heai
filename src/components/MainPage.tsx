    ...
    <div className="flex flex-col overflow-y-auto w-full scrollbar-hide pb-[120px]">
      ...
      <div className="box-border content-stretch flex flex-col gap-[40px] items-center justify-center overflow-clip px-0 py-[20px] w-full">
        ...
        <Todolist
          routines={routines}
          toggleRoutine={toggleRoutine}
          onAddRoutine={() => navigate('/routine')}
          percentage={percentage}
        />

        {/* Weight & Item Cards */}
        <div className="content-stretch flex items-center justify-between relative shrink-0 w-[343px]">
          <WeightCard onClick={() => navigate('/weight')} />
          <ItemCard />
        </div>

        {/* Sleep Record */}
        <SleepCard onClick={() => navigate('/sleep')} />

        {/* Steps Counter */}
        <StepsCard onClick={() => navigate('/steps')} />

        {/* Community Banner */}
        <Banner onClick={() => navigate('/ranking')} />
      </div>
    </div>
  );
}
...
function LvIcon({ navigate }: { navigate: (path: string) => void }) {
  return (
...
function StepsCard({ onClick }: { onClick: () => void }) {
  return (
    ...
    >
      ...
      <div className="absolute content-stretch flex flex-col items-start left-[21px] top-[21px] w-[115px]">
